...
namespace scripting.iOS
{
    ...
    public class IAP
    {
        ...
        public static void ContinuePurchaseOrRestore(NSDictionary info)
        {
            if (info == null || info.Count == 0)
            {
                if (!m_restoreCalled)
                {
                    Console.WriteLine("Need to call Restore before buying.");
                    Restore(m_buyingProductId);
                    return;
                }
        ...
        public static void Purchase(string productId)
        {
            ...
            m_requestObserver = NSNotificationCenter.DefaultCenter.AddObserver(
                ...
                (notification) => {
                  string errorStr = string.IsNullOrWhiteSpace(LastError) ? IAP.GetStringFromNotification(notification) :
                    LastError;
        ...
        public static void Restore(string productId = null)
        {
            m_restoreCalled = true;
            m_buyingProductId = (NSString)productId;
        ...
        {
            string desc = "";
            if (m_id2Description.TryGetValue(productID, out desc))
            {
                return desc;
            }
            return productID;
        ...
        public static string GetStringFromNotification(NSNotification notification)
        {
            var userInfo = notification.UserInfo;
            string errorStr = "Couldn't access App Store";
            NSObject descr = null;
            if (userInfo != null && userInfo.TryGetValue((NSString)"error", out descr))
            {
                errorStr = descr.Description;
                errorStr = errorStr.Replace("\"", "");
            }
            Console.WriteLine("Purchase Request Failed: {0}", errorStr);
    ...
    public class PurchaseManager : SKProductsRequestDelegate
    {
        ...
        public override void RequestFailed(SKRequest request, NSError error)
        {
            ...
            IAP.LastError = error.LocalizedDescription;
            NSNotificationCenter.DefaultCenter.PostNotificationName(InAppPurchaseManagerRequestFailedNotification,
                                                                    this, userInfo);
        }
