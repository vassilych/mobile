...
// IMPORTANT - it works with Syncfusion 17.1.0.38.
...
namespace scripting.iOS
{
    public class SfWidget : iOSVariable
    {
            ...
            base(UIType.CUSTOM, name)
        {
            ...
            switch (type)
            {
                ...
                case SyncFusionType.CALENDAR:
                    CreateCalendar();
                    break;
                case SyncFusionType.COLUMN_GRAPH:
        ...
        public override iOSVariable GetWidget(string widgetType, string widgetName, string initArg, CGRect rect)
        {
            switch (widgetType)
            {
                ...
                case "SfColumnGraph":
                    return new SfWidget(SfWidget.SyncFusionType.COLUMN_GRAPH, widgetName, initArg, rect);
            ...
            else if (m_chart != null)
            {
                ...
                else if (SfType == SyncFusionType.SPLINE_GRAPH)
                {
                    ...
                    if (!string.IsNullOrEmpty(extra))
                    {
                        ...
                        series.Color = UtilsiOS.String2Color(colors[0]);
                        if (colors.Length > 1)
                        {
                ...
                else if (SfType == SyncFusionType.COLUMN_GRAPH)
                {
            ...
            else if (m_chart != null)
            {
                ...
                switch (arg1)
                {
                    ...
                    case "y_interval":
                        ...
                        break;
                    case "tooltip":
                        m_chart.Delegate = new ChartTooltipDelegate(m_chart, arg2);
                        break;
                    case "tooltip_image":
                        Utils.CheckNotNull(m_chart.Delegate, "ChartTooltipDelegate", null);
                        ((ChartTooltipDelegate)m_chart.Delegate).SetImage(arg2);
                        break;
                    case "tooltip_image_size":
                        Utils.CheckNotNull(m_chart.Delegate, "ChartTooltipDelegate", null);
                        ((ChartTooltipDelegate)m_chart.Delegate).SetImageSize(arg2);
                        break;
                    case "tooltip_font":
                        Utils.CheckNotNull(m_chart.Delegate, "ChartTooltipDelegate", null);
                        ((ChartTooltipDelegate)m_chart.Delegate).SetFont(arg2);
                        break;
                    case "tooltip_bgcolor":
                        Utils.CheckNotNull(m_chart.Delegate, "ChartTooltipDelegate", null);
                        ((ChartTooltipDelegate)m_chart.Delegate).SetBGColor(arg2);
                        break;
                    case "tooltip_xprefix":
                        Utils.CheckNotNull(m_chart.Delegate, "ChartTooltipDelegate", null);
                        ((ChartTooltipDelegate)m_chart.Delegate).SetXPrefix(arg2);
                        break;
                    case "tooltip_yprefix":
                        Utils.CheckNotNull(m_chart.Delegate, "ChartTooltipDelegate", null);
            ...
            else if (m_stepper != null)
            {
                switch (arg1)
                {
                    ...
                    case "buttons":
                        ...
                        {
                            case "left":
                                m_stepper.SpinButtonAlignment = SFNumericUpDownSpinButtonAlignment.Left;
                                break;
                            case "right":
                                m_stepper.SpinButtonAlignment = SFNumericUpDownSpinButtonAlignment.Right;
                                break;
                            default:
                                m_stepper.SpinButtonAlignment = SFNumericUpDownSpinButtonAlignment.Both;
                                break;
                        }
                        break;
                }
    ...
    public class ChartTooltipDelegate : SFChartDelegate
    {
        ...
        {
            var sizes = sizesStr.Split(new char[] { ',', ':' });
            m_chart = chart;
            m_width = (float)Utils.ConvertToDouble(sizes[0]);
            m_height = (float)Utils.ConvertToDouble(sizes[1]);

            SetFont();
            SetBGColor();
        }

        public void SetBGColor(string bgColorStr = "deep_pink")
        {
            var bgColor = UtilsiOS.String2Color(bgColorStr);

            if (m_behavior != null)
            {
                m_chart.RemoveChartBehavior(m_behavior);
            }

            m_behavior = new SFChartTooltipBehavior();
            m_behavior.BackgroundColor = bgColor;
            m_chart.AddChartBehavior(m_behavior);
        }

        public void SetFontSize(float fontSize)
        {
            string fontName = m_font != null ? m_font.Name : "Helvetica";
            m_font = UIFont.FromName(fontName, fontSize);
        }
        public void SetFontColor(string strColor = "snow")
        {
            m_textColor = UtilsiOS.String2Color(strColor);
        }
        public void SetFontName(string fontName)
        {
            m_font = UIFont.FromName(fontName, 10);
        }
        public void SetFont(string str = "snow:10:Helvetica")
        {
            var parts = str.Split(new char[] { ',', ':' });
            m_textColor = UtilsiOS.String2Color(parts[0]);
            var fontSize = parts.Length < 2 ? 10f : (float)Utils.ConvertToDouble(parts[1]); ;
            var fontName = parts.Length < 3 ? "Helvetica" : parts[2];

            m_font = UIFont.FromName(fontName, fontSize);
        }

        public void SetImage(string imageName, int imageWidth = 30, int imageHeight = 30)
        {
            if (!string.IsNullOrWhiteSpace(imageName))
            {
                m_image       = UtilsiOS.LoadImage(imageName);
        ...
        public void SetImageSize(string sizesStr = "30:30")
        {
            var sizes = sizesStr.Split(new char[] { ',', ':' });
            m_imageWidth = (float)Utils.ConvertToDouble(sizes[0]);
            m_imageHeight = (float)Utils.ConvertToDouble(sizes[sizes.Length - 1]);
        }
        public void SetXPrefix(string prefix)
        {
            m_xPrefix = prefix;
        }
        public void SetYPrefix(string prefix)
        {
            m_yPrefix = prefix;
        }
        ...
        public override void WillShowTooltip(SFChart chart, SFChartTooltip tooltipView)
        {
            ...
            customView.Frame  = new CGRect(0, 0, m_width, m_height);

            if (m_image != null)
            {
                UIImageView imageView = new UIImageView();
                imageView.Frame = new CGRect(0, 0, m_imageWidth, m_imageHeight);
